			...
			<view v-for="(item,index) in list" :key="index">
				<view class="bg-white margin-sm padding-sm solids-bottom radius text-content">
					<view class="flex justify-between">
						...
						<view><text class="icon-caipin padding-right-sm"></text><text
								class="text-bold solids-bottom">{{ item.caipinOrderUuidNumber }}</text></view>
					</view>
					<view class="bg-white margin-xs padding-xs  radius text-content">
						<view class="flex padding-top-sm solids-top">
							<image :src="item.caipinPhoto" style="flex-shrink: 0;" mode="aspectFill"
					...
					<view class="flex justify-end" style="font-size: 24upx;">
						...
						<view v-if="item.caipinOrderTypes == 3">
							<view @tap="receiving(item.id)" class="round cu-btn lines-grey mid  margin-right-sm">已上餐
							</view>
						</view>
					</view>
				</view>
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							var returnVal = await _this.$api.requestCondition("caipinOrder", "refund",
								value);
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							var returnVal = await _this.$api.requestCondition("caipinOrder", "deliver",
								value);
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							var returnVal = await _this.$api.requestCondition("caipinOrder", "receiving",
								order);
